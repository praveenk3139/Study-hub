...
router.post('/forgot-password', (req, res) => {
  ...
  const user = db.prepare('SELECT id, username, email FROM users WHERE email = ? OR username = ?').get(email, email);
  if (!user) {
    return res.status(404).json({ error: 'No account found registered with this email or username' });
  }
