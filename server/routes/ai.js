...
// Universal Question Answer Solver using AI API Keys
router.post('/solve-question', optionalAuth, async (req, res) => {
  try {
    const userId = req.user ? req.user.id : 1;
    const { question_text, marks, subject_name } = req.body || {};

    if (!question_text) {
      return res.status(400).json({ error: 'question_text parameter is required' });
    }
    ...
    try {
      ...
      if (userSettings && userSettings.api_keys_json) {
        const keys = JSON.parse(userSettings.api_keys_json);
        userCustomKey = keys.gemini || keys.mistral || null;
      }
    } catch (e) {}
...
4. Conclude with key revision takeaways for university students.`;
    ...
    const assistantReply = await generateAIResponse(prompt, {
      subject: subjectStr,
      provider: 'gemini',
    ...
    console.error('Error generating AI question answer:', err);
