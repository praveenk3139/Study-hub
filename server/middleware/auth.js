...
function optionalAuth(req, res, next) {
  ...
  // Default to first active user if no token provided
  if (!req.user) {
    const defaultUser = db.prepare('SELECT id, username, email, role, is_blocked FROM users WHERE is_blocked = 0 ORDER BY id ASC LIMIT 1').get();
