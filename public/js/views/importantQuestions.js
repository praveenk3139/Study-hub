...
const ImportantQuestionsView = {
    ...
    container.innerHTML = `
      ...
      <div class="card" style="padding: 18px; margin-bottom: 24px;">
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 14px;">
          ...
          <!-- PDF Source & Unit Selectors -->
          <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
            <div style="display: flex; gap: 8px; align-items: center;">
              <span style="font-size: 12px; font-weight: 700; text-transform: uppercase; color: var(--text-muted);">
                📚 Book PDF / Source:
              </span>
              <select id="pdf-source-filter-select" class="select" style="width: 170px; padding: 6px 10px; font-size: 12px;" onchange="ImportantQuestionsView.filterPdf(this.value)">
                <option value="">All Sources / PDFs</option>
              </select>
            </div>
            ...
            <div style="display: flex; gap: 8px; align-items: center;">
              ...
              <select class="select" style="width: 130px; padding: 6px 10px; font-size: 12px;" onchange="ImportantQuestionsView.filterUnit(this.value)">
  ...
  async loadQuestions() {
    ...
    if (this.activePdfFilter) url += `pdf_id=${this.activePdfFilter}&`;

    try {
      const res = await api.get(url);

      const pdfSelect = document.getElementById('pdf-source-filter-select');
      if (pdfSelect && res.pdf_sources) {
        const currentVal = pdfSelect.value;
        pdfSelect.innerHTML = `<option value="">All Sources / PDFs</option>` +
          res.pdf_sources.map(p => `<option value="${p.id}" ${String(p.id) === String(this.activePdfFilter) ? 'selected' : ''}>📄 ${p.title}</option>`).join('');
      }

      if (res.questions.length === 0) {
        listEl.innerHTML = `
          <div class="card" style="text-align: center; padding: 40px;">
            <h3>No questions found matching current filters</h3>
            <p style="color: var(--text-muted); margin-top: 6px;">Try resetting source PDF, mark, or unit criteria.</p>
          </div>
        `;
        return;
      }
        ...
        <div class="card" style="padding: 20px; border-left: 4px solid ${q.marks >= 10 ? 'var(--accent-purple)' : 'var(--accent-cyan)'};">
          <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 14px;">
            <div style="flex: 1;">
              ...
              <details style="cursor: pointer;" id="details-q-${q.id}">
                ...
                <div id="answer-box-${q.id}" style="margin-top: 10px; background: rgba(0, 0, 0, 0.4); border: 1px solid var(--border-subtle); padding: 16px; border-radius: var(--radius-md); font-size: 13.5px; line-height: 1.7; color: var(--text-primary); white-space: pre-line;">
                  ${q.sample_answer || 'Click "Generate AI Answer" button to construct a custom university-grade model solution using AI API keys.'}
                </div>
              </details>
            </div>

            <div style="display: flex; flex-direction: column; gap: 8px;">
              <button class="btn btn-primary btn-sm" onclick="ImportantQuestionsView.generateAiAnswer(${q.id}, \`${(q.question_text || '').replace(/`/g, '')}\`, ${q.marks}, '${(q.subject_name || 'Data Structures').replace(/'/g, '')}')">
                ✨ Generate AI Answer
              </button>
  ...
  activePdfFilter: null,

  filterPdf(pdfId) {
    this.activePdfFilter = pdfId ? parseInt(pdfId) : null;
    this.loadQuestions();
  },
  ...
  askCoachAboutQuestion(qText) {
    ...
    setTimeout(() => {
      if (typeof AiAgentView !== 'undefined' && AiAgentView.sendQuickPrompt) {
        AiAgentView.sendQuickPrompt(`Give me a structured university exam answer with diagrams for: "${qText}"`);
      }
    }, 200);
  },

  async generateAiAnswer(qId, qText, marks, subjectName) {
    const box = document.getElementById(`answer-box-${qId}`);
    const details = document.getElementById(`details-q-${qId}`);
    if (details) details.open = true;

    if (box) {
      box.innerHTML = `<div style="display: flex; align-items: center; gap: 10px; color: var(--accent-cyan);"><div class="pulse-indicator"></div> Generating tailored ${marks}-mark university answer using AI API...</div>`;
    }

    try {
      const res = await api.post('/important-questions/generate-answer', {
        question_id: qId,
        question_text: qText,
        marks: marks,
      ...
      if (box) {
        box.innerHTML = res.answer;
        box.style.border = '1px solid var(--accent-purple)';
      }
      showToast(`AI ${marks}-Mark Model Answer Generated Successfully!`, 'success');
      if (typeof playAudioChime === 'function') playAudioChime('chime');
    } catch (e) {
      if (box) {
        box.innerHTML = `<div style="color: var(--accent-rose);">Could not generate AI answer. Please check your AI API key in Settings.</div>`;
      }
    }
  },
