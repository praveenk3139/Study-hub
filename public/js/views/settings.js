...
const SettingsView = {
    ...
    try {
      ...
      container.innerHTML = `
        <div style="display: flex; flex-direction: column; gap: 24px; max-width: 860px;">
          <!-- 0. Theme & Visual Appearance -->
          <div class="card" style="border-top: 3px solid var(--accent-purple);">
            <div class="card-header">
              <div class="card-title">🎨 Visual Theme & Appearance</div>
              <span class="badge badge-low" id="theme-active-badge">
                ${App.currentTheme === 'dark' ? '🌙 Dark Mode Active' : '☀️ Light Mode Active'}
              </span>
            </div>
            <p style="font-size: 13px; color: var(--text-secondary); margin-bottom: 16px;">
              Choose between soft light mode or sleek dark mode to suit your study environment and reduce eye strain.
            </p>
            <div class="grid-3" style="margin-bottom: 12px;">
              <div id="theme-option-light" onclick="App.applyTheme('light')" style="cursor: pointer; padding: 16px; border-radius: var(--radius-md); border: 2px solid ${App.currentTheme === 'light' ? 'var(--accent-blue)' : 'var(--border-subtle)'}; background: var(--bg-surface-elevated); text-align: center; transition: all 0.2s;">
                <div style="font-size: 28px; margin-bottom: 6px;">☀️</div>
                <div style="font-weight: 700; font-size: 14px; color: var(--text-primary);">Light Mode</div>
                <div style="font-size: 11px; color: var(--text-secondary); margin-top: 4px;">Clean & bright daytime UI</div>
              </div>

              <div id="theme-option-dark" onclick="App.applyTheme('dark')" style="cursor: pointer; padding: 16px; border-radius: var(--radius-md); border: 2px solid ${App.currentTheme === 'dark' ? 'var(--accent-blue)' : 'var(--border-subtle)'}; background: var(--bg-surface-elevated); text-align: center; transition: all 0.2s;">
                <div style="font-size: 28px; margin-bottom: 6px;">🌙</div>
                <div style="font-weight: 700; font-size: 14px; color: var(--text-primary);">Dark Mode</div>
                <div style="font-size: 11px; color: var(--text-secondary); margin-top: 4px;">Sleek & eye-friendly night UI</div>
              </div>

              <div onclick="App.toggleTheme()" style="cursor: pointer; padding: 16px; border-radius: var(--radius-md); border: 2px solid var(--border-subtle); background: var(--bg-surface-elevated); text-align: center; transition: all 0.2s;">
                <div style="font-size: 28px; margin-bottom: 6px;">🌗</div>
                <div style="font-weight: 700; font-size: 14px; color: var(--text-primary);">Quick Toggle</div>
                <div style="font-size: 11px; color: var(--text-secondary); margin-top: 4px;">Switch theme instantly</div>
              </div>
            </div>
          </div>
  ...
  updateThemeUI(theme) {
    const badgeEl = document.getElementById('theme-active-badge');
    if (badgeEl) {
      badgeEl.innerText = theme === 'dark' ? '🌙 Dark Mode Active' : '☀️ Light Mode Active';
    }

    const lightBox = document.getElementById('theme-option-light');
    const darkBox = document.getElementById('theme-option-dark');
    if (lightBox && darkBox) {
      lightBox.style.borderColor = theme === 'light' ? 'var(--accent-blue)' : 'var(--border-subtle)';
      darkBox.style.borderColor = theme === 'dark' ? 'var(--accent-blue)' : 'var(--border-subtle)';
    }
  }
};
