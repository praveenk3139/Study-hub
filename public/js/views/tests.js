...
const TestsView = {
  ...
  async render() {
    ...
    container.innerHTML = `
      ...
      <!-- Separate PDF & Question Paper Diagnostic Test Generator Card -->
      <div class="card" style="margin-bottom: 24px; border-left: 4px solid var(--accent-cyan); background: var(--bg-surface-elevated);">
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 14px;">
          <div>
            <h3 style="font-size: 16px; font-weight: 800; color: var(--accent-cyan);">🎯 Separate Test Generator by Book PDF / Question Paper</h3>
            <p style="font-size: 13px; color: var(--text-secondary); margin-top: 2px;">
              Select any specific uploaded Book PDF, Question Bank, or Question Paper to generate a random 5-question AI test.
            </p>
          </div>
          <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
            <select id="test-pdf-selector" class="select" style="width: 220px; padding: 8px 12px; font-size: 13px;">
              <option value="">Select Document Source...</option>
            </select>
            <button class="btn btn-primary" onclick="TestsView.generateSelectedDocumentTest()">
              ⚡ Launch Random AI Test
            </button>
          </div>
        </div>
      </div>
  ...
  async loadCatalog() {
    ...
    try {
      const [testsRes, historyRes, pdfsRes, pyqRes] = await Promise.all([
        api.get('/tests'),
        api.get('/tests/history/summary'),
        api.get('/pdf').catch(() => ({ documents: [] })),
        api.get('/question-papers').catch(() => ({ papers: [] }))
      ]);

      const selectEl = document.getElementById('test-pdf-selector');
      if (selectEl) {
        let opts = `<option value="">Select Document Source...</option>`;
        if (pdfsRes.documents && pdfsRes.documents.length > 0) {
          opts += `<optgroup label="📚 Book PDFs & Study Notes">`;
          pdfsRes.documents.forEach(d => {
            opts += `<option value="pdf_${d.id}">📄 ${d.title}</option>`;
          });
          opts += `</optgroup>`;
        }
        if (pyqRes.papers && pyqRes.papers.length > 0) {
          opts += `<optgroup label="📜 University Question Papers">`;
          pyqRes.papers.forEach(p => {
            opts += `<option value="pyq_${p.id}">📝 ${p.exam_term || p.filename}</option>`;
          });
          opts += `</optgroup>`;
        }
  ...
  async startTest(testId) {
    try {
      ...
      if (!res || !res.test || !res.questions || res.questions.length === 0) {
        showToast('This test has no questions available. Please select a Document Source or click "Generate AI Diagnostic Sprint".', 'warning');
        return;
      }
      ...
      this.secondsLeft = (res.test.duration_mins || 20) * 60;
      ...
      showToast('Could not initiate test session: ' + (e.message || 'Error'), 'error');
    }
  },

  renderTakingTest() {
    const container = document.getElementById('view-content');
    if (!this.questions || this.questions.length === 0 || !this.activeTest) {
      container.innerHTML = `
        <div class="card" style="padding: 40px; text-align: center;">
          <h3>No test questions currently active.</h3>
          <button class="btn btn-primary" style="margin-top: 16px;" onclick="TestsView.render()">Return to Test Center</button>
        </div>
      `;
      return;
    }

    const q = this.questions[this.currentQIndex] || this.questions[0];
        ...
        <div style="display: flex; flex-direction: column; gap: 14px; margin-bottom: 28px;">
          ${report.evaluated_answers.map((ans, idx) => `
            <div style="background: var(--bg-base); padding: 16px; border-radius: var(--radius-md); border-left: 4px solid ${ans.is_correct ? 'var(--accent-emerald)' : 'var(--accent-rose)'};">
              ...
              <div style="font-size: 12.5px; color: var(--accent-cyan); margin-top: 4px; margin-bottom: 8px;">
                ${ans.ai_feedback}
              </div>
              <button class="btn btn-primary btn-sm" style="margin-top: 6px;" onclick="TestsView.generateAiAnswerForQuestion(\`${(ans.question_text || '').replace(/`/g, '\\`')}\`, ${ans.max_marks || 5}, 'test-report-ai-${idx}', 'Computer Science & Engineering')">
                ⚡ Generate Full AI Model Solution (Gemini API)
              </button>
              <div id="test-report-ai-${idx}" style="display: none;"></div>
            </div>
          `).join('')}
        </div>
  ...
  async generateSelectedDocumentTest() {
    const selector = document.getElementById('test-pdf-selector');
    if (!selector || !selector.value) {
      showToast('Please select a document source (Book PDF or Question Paper)', 'warning');
      return;
    }
    const val = selector.value;
    const [type, id] = val.split('_');
    const payload = type === 'pdf' ? { pdf_id: parseInt(id) } : { paper_id: parseInt(id) };

    showToast('AI is generating a random 5-question Diagnostic Test from selected source...', 'info');
    try {
      const res = await api.post('/tests/generate-pdf-test', payload);
      showToast(res.message || 'Diagnostic Test generated successfully!', 'success');
      this.startTest(res.test_id);
    } catch (e) {
      showToast('Failed to generate test: ' + (e.message || 'Unknown error'), 'error');
    }
  },
  ...
  async generateAiAnswerForQuestion(qText, marks, containerId, subject = 'Computer Science & Engineering') {
    ...
    container.innerHTML = `<div style="padding: 14px; color: var(--accent-cyan); font-size: 13.5px;"><div class="pulse-indicator"></div> Generating Model Answer using Google Gemini API key...</div>`;
      ...
      container.innerHTML = `
        <div style="background: var(--bg-surface-elevated); border: 1px solid var(--accent-purple); border-radius: var(--radius-md); padding: 18px; margin-top: 10px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; border-bottom: 1px solid var(--border-subtle); padding-bottom: 8px;">
            ...
            <span class="badge badge-high">Verified AI Answer</span>
          </div>
