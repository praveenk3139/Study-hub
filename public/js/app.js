...
const App = {
  ...
  routes: {
    ...
    'my-study': MyStudyView,
    'weak-topics': WeakTopicsView,
    'revision-session': RevisionSessionView,
    'exam-readiness': ExamReadinessView,
    'flashcards': FlashcardsView,
    'image-solver': ImageSolverView,
    'study-rooms': StudyRoomsView,
    'admin': AdminView,
  ...
  applyTheme(theme) {
    ...
    if (typeof SettingsView !== 'undefined' && typeof SettingsView.updateThemeUI === 'function') {
      SettingsView.updateThemeUI(theme);
    }
  },
  ...
  async init() {
    // 0. Initialize theme (Light / Dark)
    this.initTheme();

    // 0. Setup Global Link Interceptor for Focus Shield
    this.setupGlobalLinkInterceptor();
  ...
  // Intercept external links globally to enforce Focus Shield
  setupGlobalLinkInterceptor() {
    document.addEventListener('click', (e) => {
      ...
      try {
        ...
        const blockedSites = (typeof FocusShieldView !== 'undefined' && FocusShieldView.blockedSites && FocusShieldView.blockedSites.length > 0)
          ? FocusShieldView.blockedSites.map(b => b.domain.toLowerCase())
          : ['instagram.com', 'facebook.com', 'tiktok.com', 'x.com', 'twitter.com', 'reddit.com', 'netflix.com', 'twitch.tv'];

        const isBlocked = blockedSites.some(domain => host.includes(domain));
        ...
        if (isBlocked) {
          ...
          api.post('/focus/distraction-event', {
            attempted_target: href,
            reason: `Attempted navigation to ${host} while Focus Shield was active`
          }).catch(() => {});

          this.showFocusBlockerModal(host, href);
  ...
  showFocusBlockerModal(host, targetUrl) {
    const existing = document.getElementById('focus-blocker-modal');
    if (existing) existing.remove();

    const modal = document.createElement('div');
    modal.id = 'focus-blocker-modal';
    modal.className = 'modal-overlay';
    modal.innerHTML = `
      <div class="modal-content" style="max-width: 480px; text-align: center; border-top: 4px solid var(--accent-rose); padding: 30px;">
        <div style="font-size: 48px; margin-bottom: 12px;">🛡️</div>
        <div class="badge badge-high" style="margin-bottom: 12px;">FOCUS SHIELD BLOCKED ACCESS</div>
        <h3 style="font-size: 20px; font-weight: 800; margin-bottom: 8px;">Access to ${host} Filtered</h3>
        <p style="font-size: 13px; color: var(--text-secondary); line-height: 1.6; margin-bottom: 20px;">
          Navigation to <strong>${targetUrl}</strong> was blocked by StudyFlow AI Focus Shield during your active study session to protect your focus and academic streak.
        </p>

        <div style="background: var(--bg-base); padding: 14px; border-radius: var(--radius-md); font-size: 12.5px; color: var(--text-muted); margin-bottom: 24px;">
          💡 <strong>Pro Tip:</strong> Completing your 45-minute focus session awards <strong>+50 XP</strong> and maintains your daily streak!
        </div>

        <div style="display: flex; gap: 12px; justify-content: center;">
          <button class="btn btn-primary" onclick="this.closest('.modal-overlay').remove(); window.location.hash = '#focus-shield';">
            🎯 Return to Focus Shield Workspace
          </button>
        </div>
      </div>
    `;
