...
async function runTests() {
  ...
  // 5. Strict Study Planner & Excel Sheet Operations
  await test('GET /api/planner, /sample-excel & /export-excel (Strict Planner & Excel Sync)', async () => {
    const res = await request({
    ...
    const sampleRes = await request({
      hostname: 'localhost', port: TEST_PORT, path: '/api/planner/sample-excel', method: 'GET'
    });
    if (sampleRes.status !== 200) throw new Error(`Sample excel download failed with status ${sampleRes.status}`);

    const exportRes = await request({
      hostname: 'localhost', port: TEST_PORT, path: '/api/planner/export-excel', method: 'GET',
      headers: { 'Authorization': `Bearer ${authToken}` }
    });
    if (exportRes.status !== 200) throw new Error(`Export excel failed with status ${exportRes.status}`);

    const aiPlanRes = await request({
      hostname: 'localhost', port: TEST_PORT, path: '/api/planner/generate-ai', method: 'POST',
      headers: { 'Authorization': `Bearer ${authToken}`, 'Content-Type': 'application/json' }
    }, { available_hours: 4 });
    if (aiPlanRes.status !== 200 || !aiPlanRes.data.success) throw new Error(`AI Plan generation failed: ${JSON.stringify(aiPlanRes.data)}`);
  });
  ...
  // 10. Weak Topics & Revision Engine API Test
  await test('GET /api/weak-topics & /api/revision/Trees', async () => {
    const wtRes = await request({
      hostname: 'localhost', port: TEST_PORT, path: '/api/weak-topics', method: 'GET',
      headers: { 'Authorization': `Bearer ${newAuthToken}` }
    });
    if (wtRes.status !== 200 || !wtRes.data.topics) throw new Error('Weak topics fetch failed');

    const revRes = await request({
      hostname: 'localhost', port: TEST_PORT, path: '/api/revision/Trees', method: 'GET',
      headers: { 'Authorization': `Bearer ${newAuthToken}` }
    });
    if (revRes.status !== 200 || revRes.data.topic !== 'Trees') throw new Error('Revision session fetch failed');

    const retRes = await request({
      hostname: 'localhost', port: TEST_PORT, path: '/api/revision/Trees/retest', method: 'POST',
      headers: { 'Authorization': `Bearer ${newAuthToken}`, 'Content-Type': 'application/json' }
    }, { score: 3, total: 3 });
    if (retRes.status !== 200 || retRes.data.retest_score !== 100) throw new Error('Retest submission failed');
  });

  // 11. My Study, Flashcards & Study Rooms API Test
  await test('GET /api/my-study/subjects & /api/flashcards/decks & /api/study-rooms', async () => {
    const msRes = await request({
      hostname: 'localhost', port: TEST_PORT, path: '/api/my-study/subjects', method: 'GET',
      headers: { 'Authorization': `Bearer ${newAuthToken}` }
    });
    if (msRes.status !== 200 || !msRes.data.subjects) throw new Error('My study subjects fetch failed');

    const fcRes = await request({
      hostname: 'localhost', port: TEST_PORT, path: '/api/flashcards/decks', method: 'GET',
      headers: { 'Authorization': `Bearer ${newAuthToken}` }
    });
    if (fcRes.status !== 200 || !fcRes.data.decks) throw new Error('Flashcard decks fetch failed');

    const srRes = await request({
      hostname: 'localhost', port: TEST_PORT, path: '/api/study-rooms', method: 'GET',
      headers: { 'Authorization': `Bearer ${newAuthToken}` }
    });
    if (srRes.status !== 200 || !srRes.data.rooms) throw new Error('Study rooms fetch failed');
  });

  // 12. Generate Random Diagnostic Test by Document PDF/PYQ
  await test('POST /api/tests/generate-pdf-test (Document Diagnostic Generator)', async () => {
    const pdfTestRes = await request({
      hostname: 'localhost', port: TEST_PORT, path: '/api/tests/generate-pdf-test', method: 'POST',
      headers: { 'Authorization': `Bearer ${newAuthToken}`, 'Content-Type': 'application/json' }
    }, { pdf_id: 1 });
    if (pdfTestRes.status !== 201 || !pdfTestRes.data.test_id) {
      throw new Error(`PDF Test generation failed: ${JSON.stringify(pdfTestRes.data)}`);
    }
  });
